 ...
 * @version 4.0
 * 
 * Modified by Ting Zhang for CSC2206 Lab 7
 */
...
public Iterator<T> iteratorLevelOrder() 
   {
     LinkedQueue<BinaryTreeNode> nodes = new LinkedQueue<BinaryTreeNode>();
     ArrayList<T> results = new ArrayList<T>();
     nodes.enqueue(root);
     while(!nodes.isEmpty()) {
    	BinaryTreeNode<T> elem1 = nodes.dequeue();
    	 if (elem1 != null) {
    		 results.add(elem1.getElement()); 
    		 nodes.enqueue(elem1.getLeft()); //Depending on the number of children and parents
    		 nodes.enqueue(elem1.getRight()); //in each of the left and right nodes, there will be
    	 }									  //several null statements
    	 else
    		 results.add(null);	// null is added to results when either the child or parent  
     }							// does not exist on either the left or the right
